...
export class AuthenticationClient {
  ...
  getAuthorizeRedirect(scopes: string[], redirectUri: string): string {
    return `${this.host}/${RootPath}/authorize?response_type=code&client_id=${
      this.client_id
  ...
  async getUserProfile(token: string): Promise<IUserProfile> {
    ...
    const resp = await axios.get(
      this.host + '/userprofile/v1/users/@me',
      config
    );
    return resp.data;
  }
